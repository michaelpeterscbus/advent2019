...
public class Part2 {
	...
	private static List<Point> plotPoints(String[] wire) {
		List<Point> wirePoints = new ArrayList<Point>();
		Point currentPoint = new Point(0, 0);
		for (int i = 0; i < wire.length; i++) {
			int distance = Integer.parseInt(wire[i].substring(1));
			if (wire[i].startsWith("R")) {
				for (int j = 1; j <= distance; j++) {
					Point nextPoint = new Point(currentPoint.x + j, currentPoint.y);
					wirePoints.add(nextPoint);
				}
				currentPoint.setLocation(currentPoint.x + distance, currentPoint.y);
			} else if (wire[i].startsWith("D")) {
				for (int j = 1; j <= distance; j++) {
					Point nextPoint = new Point(currentPoint.x, currentPoint.y - j);
					wirePoints.add(nextPoint);
				}
				currentPoint.setLocation(currentPoint.x, currentPoint.y - distance);
