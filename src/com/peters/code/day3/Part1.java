...
public class Part1 {
	...
	public static void main(String[] args) {
		...
		Set<Point> wire1Points = plotPoints(wire1);
		Set<Point> wire2Points = plotPoints(wire2);
		wire1Points.retainAll(wire2Points);

		int closestIntersection = 2147483647;
		Iterator iterator = wire1Points.iterator();
		while (iterator.hasNext()) {
			Point currentPoint = (Point)iterator.next();
			int manhattanDistance = Math.abs(currentPoint.x) + Math.abs(currentPoint.y);
			if (manhattanDistance < closestIntersection && manhattanDistance != 0) {
	...
	private static Set<Point> plotPoints(String[] wire) {
		Set<Point> wirePoints = new HashSet<Point>();
